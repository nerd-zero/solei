...
from solei.cli.listener import LISTENER_KEY_PREFIX, has_active_listener
...
def mock_organization_service(mocker: MockerFixture, org_id: uuid.UUID) -> MagicMock:
    ...
    mock_org.id = org_id
    return mocker.patch(
        "solei.cli.endpoints.organization_service.get",
...
class TestListenEndpoint:
    async def test_mark_active_on_connect(
        ...
        from solei.cli.endpoints import listen
