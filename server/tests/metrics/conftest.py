...
from solei.models import Customer, Event, Order, Organization, Product, Subscription
...
async def create_events_for_fixtures(
    ...
    """Create Events corresponding to orders/subscriptions for Tinybird."""
    events: list[Event] = []
    ...
    for order in orders.values():
        ...
        if product is not None:
            paid_metadata["billing_type"] = product.billing_type.value
        ev = await create_event(
            save_fixture,
            organization=organization,
            customer=customer,
        ...
        if order.subscription_id:
            balance_metadata["subscription_id"] = str(order.subscription_id)
        if product is not None:
            balance_metadata["billing_type"] = product.billing_type.value
        ev = await create_event(
            save_fixture,
            organization=organization,
            customer=customer,
...
async def metrics_events(
    ...
    """Create Events corresponding to the test fixtures for Tinybird."""
...
async def metrics_backend(
    ...
    """Fixture that ingests test data into Tinybird for metrics tests.
...
async def metrics_backend_helper(
    ...
    """Fixture for tests with custom fixtures.
