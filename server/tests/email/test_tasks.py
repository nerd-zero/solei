...
from solei.email.sender import SendEmailError
from solei.email.tasks import email_send
from solei.enums import EmailSender
from solei.models.email_log import EmailLog, EmailLogStatus
...
class TestEmailSend:
    async def test_successful_send(
        ...
        mock_send = mocker.patch(
            "solei.email.tasks.email_sender.send",
        ...
        await email_send(
            ...
            from_name="Solei",
            from_email_addr="noreply@solei.to",
            email_headers=None,
            reply_to_name=None,
    ...
    async def test_template_email_extracts_organization_id(
        ...
        mocker.patch(
            "solei.email.tasks.render_from_json",
    ...
    async def test_processor_reflects_settings(
        ...
        mocker.patch(
            "solei.email.tasks.settings.EMAIL_SENDER",
            EmailSender.logger,
        )
    ...
    async def test_log_failure_does_not_mask_send_error(
        ...
        mocker.patch(
            "solei.email.tasks.AsyncSessionMaker",
            side_effect=RuntimeError("db down"),
        )
        log_exception = mocker.patch("solei.email.tasks.log.exception")
