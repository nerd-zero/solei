...
from solei.kit.utils import human_readable_size
...
class S3File(IDSchema, validate_assignment=True):
    ...
    def to_metadata(self) -> dict[str, str]:
        metadata = {
            "solei-id": str(self.id),
            "solei-organization-id": str(self.organization_id),
            "solei-name": self.name.encode("ascii", "ignore").decode("ascii"),
            "solei-size": str(self.size),
        }
        if self.checksum_sha256_base64:
            metadata["solei-sha256-base64"] = self.checksum_sha256_base64
        if self.checksum_sha256_hex:
            metadata["solei-sha256-hex"] = self.checksum_sha256_hex
        return metadata
    ...
    def from_head(cls, path: str, head: dict[str, Any]) -> Self:
        ...
        return cls(
            id=metadata.get("solei-id"),
            organization_id=metadata.get("solei-organization-id"),
            name=metadata.get("solei-name"),
            path=path,
            mime_type=head["ContentType"],
            size=metadata.get("solei-size"),
            storage_version=head.get("VersionId", None),
            checksum_etag=head.get("ETag", None),
            checksum_sha256_base64=metadata.get("solei-sha256-base64"),
            checksum_sha256_hex=metadata.get("solei-sha256-hex"),
