...
from solei.enums import PaymentMode
...
from solei.wallet.repository import WalletRepository, WalletTransactionRepository

log: Logger = structlog.get_logger()


class OrderDoesNotExist(SoleiError):
...
class OutdatedCheckoutIntent(SoleiError):
    """
...
class UnresolvedPaymentError(SoleiError):
        ...
        if object.OBJECT_NAME == "setup_intent":
            ...
            if checkout.status == CheckoutStatus.open:
                ...
                if (
                    ...
                    and updated_object.metadata.get("solei_trial_abuse_detected")
                    == "true"
                ):
                    raise OutdatedCheckoutIntent(checkout.id, object.id)
