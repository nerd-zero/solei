...
from solei.exceptions import (
    ResourceNotModified,
    SoleiError,
    SoleiRedirectionError,
    SoleiRequestValidationError,
)


async def polar_exception_handler(request: Request, exc: SoleiError) -> JSONResponse:
    return JSONResponse(
...
async def request_validation_exception_handler(
    request: Request, exc: RequestValidationError | SoleiRequestValidationError
) -> JSONResponse:
    return JSONResponse(
        status_code=422,
...
async def polar_redirection_exception_handler(
    request: Request, exc: SoleiRedirectionError
) -> RedirectResponse:
...
def add_exception_handlers(app: FastAPI) -> None:
    ...
    app.add_exception_handler(SoleiError, polar_exception_handler)  # type: ignore
