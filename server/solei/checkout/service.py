...
from solei.custom_field.data import validate_custom_field_data
...
from solei.discount.service import DiscountNotRedeemableError
...
from solei.integrations.stripe.utils import get_fingerprint
...
from solei.kit.operator import attrgetter
...
from solei.models.checkout_product import CheckoutProduct
from solei.models.discount import DiscountDuration
from solei.models.order import OrderBillingReasonInternal
from solei.models.product_price import ProductPriceSource
...
from solei.posthog import posthog
...
from solei.product.schemas import ProductPriceCreateList
...
from solei.tax.calculation import TaxCode
from solei.tax.calculation import tax_calculation as tax_calculation_service
from solei.tax.calculation.base import TaxCalculationLogicalError
from solei.tax.tax_id import InvalidTaxID, TaxID, to_stripe_tax_id, validate_tax_id
from solei.trial_redemption.service import trial_redemption as trial_redemption_service
...
class CheckoutError(SoleiError): ...
...
CHECKOUT_CLIENT_SECRET_PREFIX = "solei_c_"
...
class CheckoutService:
        ...
        **query_metadata: str | None,
        ...
        if is_fixed_price(price):
            ...
            if query_amount_str is not None and isinstance(query_amount_str, str):
                ...
                except (ValueError, TypeError, SoleiRequestValidationError):
                    pass
        ...
        if checkout_link.discount_id is not None:
            ...
            except SoleiRequestValidationError:
                pass
    ...
    async def _confirm_inner(
        ...
        if checkout.payment_processor == PaymentProcessor.stripe:
                ...
                if (
                    ...
                    if trial_already_redeemed:
                        ...
                        if intent is not None:
                            ...
                            trial_intent_metadata["solei_trial_abuse_detected"] = "true"
            ...
            async with self._create_or_update_customer(
                ...
                from solei.integrations.smilepay.service import (
                    smilepay_service,
                )
    ...
    def _validate_checkout_seat_constraints(
        ...
        if not is_seat_price(price):
            ...
            raise SoleiRequestValidationError(fields)
