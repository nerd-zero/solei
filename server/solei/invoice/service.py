...
from solei.integrations.aws.s3 import S3Service
from solei.kit.utils import utc_now
from solei.models import Account, Order, Payout
from solei.models.transaction import PlatformFeeType
from solei.postgres import AsyncSession
from solei.tax.calculation import TaxabilityReason
from solei.transaction.repository import TransactionRepository
...
class InvoiceService:
    ...
    async def create_payout_invoice(self, session: AsyncSession, payout: Payout) -> str:
        ...
        items = [
            InvoiceItem(
                description=f"Digital services and products resold by Solei\nFrom {earliest.strftime('%Y-%m-%d')} to {latest.strftime('%Y-%m-%d')}",
                quantity=1,
                unit_amount=gross_amount,
                amount=gross_amount,
            ),
            InvoiceItem(
                description="Solei revenue share",
                quantity=1,
        ...
        invoice = Invoice(
            ...
            + (
                "Nerd Zero is the merchant of record reselling digital services.\n"
                "Nerd Zero captures and remits international sales tax from such sales – as needed.\n"
