...
@cli.command()
@typer_async
async def batch_orders_refund(
    organization_id: UUID4 = typer.Argument(
        ..., help="Organization ID to cancel subscriptions for"
    ),
) -> None:
    """Refund all paid orders for the specified organization ID."""
    ...
    async with JobQueueManager.open(dramatiq.get_broker(), redis) as job_queue_manager:
        async with sessionmaker() as session:
            ...
            with Progress() as progress:
                ...
                total_statement = order_statement.with_only_columns(
                    func.count()
                ).order_by(None)
                total_result = await session.execute(total_statement)
                total = total_result.scalar_one()

                task = progress.add_task("[cyan]Refunding orders...", total=total)
