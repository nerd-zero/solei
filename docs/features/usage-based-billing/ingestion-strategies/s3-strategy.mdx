...
```
pnpm add @solei/ingestion @aws-sdk/client-s3
```

```typescript
import { Ingestion } from "@solei/ingestion";
import { S3Strategy } from "@solei/ingestion/strategies/S3";
...
const s3Ingestion = Ingestion({ accessToken: process.env.SOLEI_ACCESS_TOKEN })
