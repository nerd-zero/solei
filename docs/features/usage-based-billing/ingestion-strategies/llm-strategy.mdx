...
```
pnpm add @solei/ingestion ai @ai-sdk/openai
```

```typescript
import { Ingestion } from "@solei/ingestion";
import { LLMStrategy } from "@solei/ingestion/strategies/LLM";
import { generateText } from "ai";
import { openai } from "@ai-sdk/openai";

// Setup the LLM Ingestion Strategy
const llmIngestion = Ingestion({ accessToken: process.env.SOLEI_ACCESS_TOKEN })
...
Our Python SDK includes an ingestion helper and strategies for common use cases. It's installed as part of the Solei SDK.

<CodeGroup>

```bash pip
pip install solei-sdk
```
...
The ingestion helper is a simple wrapper around the Solei events ingestion API. It takes care of batching and sending events to Solei in the background, without blocking your main thread.

```python
import os
from solei_sdk.ingestion import Ingestion

ingestion = Ingestion(os.getenv("SOLEI_ACCESS_TOKEN"))
...
With our PydanticAI strategy, you can easily track the usage of LLMs and send the data to Solei for billing.

```python
import os
from solei_sdk.ingestion import Ingestion
from solei_sdk.ingestion.strategies import PydanticAIStrategy
