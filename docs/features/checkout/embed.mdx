...
Then, you should import the `SoleiEmbedCheckout` helper class and manually call `SoleiEmbedCheckout.init()`. This will add the required handlers on elements having the `data-polar-checkout` attribute.

Here is an example in React:

```ts
import { SoleiEmbedCheckout } from '@polar-sh/checkout/embed'
