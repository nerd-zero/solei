...
<img src="https://solei.to/docs/assets/features/checkout/embed/demo.png" />
...
<a
  ...
  data-solei-checkout
  data-solei-checkout-theme="light"
>
  Purchase
</a>
...
<script
  defer
  data-auto-init
  src="https://cdn.jsdelivr.net/npm/@solei/checkout@latest/dist/embed.global.js"
></script>
```

This will display a `Purchase` link which will open an inline checkout when clicked.

You can style the trigger element any way you want, as long as you keep the `data-solei-checkout` attribute.
...
<CodeGroup>
  ```bash npm
  npm install @solei/checkout
  ```
  ```bash pnpm
  pnpm add @solei/checkout
  ```
  ```bash yarn
  yarn add @solei/checkout
  ```
</CodeGroup>

Then, you should import the `SoleiEmbedCheckout` helper class and manually call `SoleiEmbedCheckout.init()`. This will add the required handlers on elements having the `data-solei-checkout` attribute.

Here is an example in React:

```ts
import { SoleiEmbedCheckout } from '@solei/checkout/embed'
import { useEffect } from 'react'

const PurchaseLink = () => {
  useEffect(() => {
    SoleiEmbedCheckout.init()
  }, [])
...
For users who need more control over the embedded checkout flow, the `SoleiEmbedCheckout` class provides several advanced features.

### Programmatically creating an embed

Instead of using declarative triggers with `data-solei-checkout` attributes, you can programmatically create and control checkout instances:
...
const openCheckout = async () => {
  ...
  try {
    ...
    const checkout = await SoleiEmbedCheckout.create(checkoutLink, {
      theme,
    });
...
const openCheckoutWithEvents = async () => {
  const checkout = await SoleiEmbedCheckout.create("__CHECKOUT_LINK__", {
    onLoaded: (event) => {
...
const CheckoutButton = () => {
  ...
  const handleCheckout = async () => {
      try {
        const checkout = await SoleiEmbedCheckout.create(
          '__CHECKOUT_LINK__',
          {
            theme: 'light',
            onLoaded: (event) => {
...
To enable wallet payment methods on your embedded checkout, please [email us](mailto:support@solei.to) with:
