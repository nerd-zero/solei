...
You can easily sell software license keys with Solei without having to deal with sales tax or hosting an API to validate them in real-time. License keys with Solei come with a lot of powerful features built-in.
...
**Enable user to deactivate instances via Solei.** Instead of building your own custom admin for customers to manage their activation instances - leave it to Solei instead.
...
It's super easy and straightforward to integrate Solei license keys into your application, library or API.
...
```bash Terminal
curl -X POST https://api.solei.to/v1/customer-portal/license-keys/activate
...
```bash Terminal
curl -X POST https://api.solei.to/v1/customer-portal/license-keys/validate
-H "Content-Type: application/json"
-d '{
  "key": "1C285B2D-6CE6-4BC7-B8BE-ADB6A7E304DA",
  "organization_id": "fda84e25-7b55-4d67-916d-60ead04ff61f",
