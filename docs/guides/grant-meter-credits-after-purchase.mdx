...
There are two ways to grant initial meter credits in Solei:
...
- A Solei account with an organization
- A meter created with **Sum** aggregation
- A product with the meter attached
- Webhooks enabled
- A Solei access token for API calls
...
```typescript icon="square-js" title="app/api/webhook/solei/route.ts"
...
<Steps>
  <Step title="Use the sandbox environment">
    Test your integration in Solei's [sandbox environment](/integrate/sandbox) to avoid affecting production data.
  </Step>
