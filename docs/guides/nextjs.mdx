---
title: "Integrate Solei with Next.js"
sidebarTitle: "Next.js"
description: "In this guide, we'll show you how to integrate Solei with Next.js."
---
...
[A complete code-example of this guide can be found on GitHub](https://github.com/nerd-zero/solei-next).

## Install the Solei JavaScript SDK

To get started, you need to install the Solei JavaScript SDK and the Solei Nextjs helper package. You can do this by running the following command:

```bash Terminal
pnpm install @solei/sdk @solei/nextjs
```
...
To authenticate with Solei, you need to create an access token, and supply it to Next.js using a `SOLEI_ACCESS_TOKEN` environment variable.

You can create an organization access token from your organization settings.

## Configuring a Solei API Client

To interact with the Solei API, you need to create a new instance of the `Solei` class. This class uses the provided access token to authenticate with the Solei API.

```typescript
// src/solei.ts
import { Solei } from "@solei/sdk";

export const api = new Solei({
  accessToken: process.env.SOLEI_ACCESS_TOKEN!,
...
import { Checkout } from "@solei/nextjs";
...
```typescript
// src/app/api/webhook/solei/route.ts
import { Webhooks } from "@solei/nextjs";
