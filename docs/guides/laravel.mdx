---
title: "Integrate Solei with Laravel"
sidebarTitle: "Laravel"
description: "In this guide, we'll show you how to integrate Solei with Laravel."
---
...
Solei Laravel Example App
--------------------------------

We've created a simple example Laravel application that you can use as a reference.

[View Code on GitHub](https://github.com/nerd-zero/solei-laravel)
...
To authenticate with Solei, you need to create an access token, and supply it to Laravel using a `POLAR_API_KEY` environment variable.

You can create an organization access token from your organization settings.

Fetching Solei Products for display
------------------------------------------
...
class ProductsController extends Controller
{
    ...
    {
        // Change from sandbox-api.solei.to -> api.solei.to when ready to go live
        // And don't forget to update the .env file with the correct POLAR_ORGANIZATION_ID and POLAR_WEBHOOK_SECRET
        $data = Http::get('https://sandbox-api.solei.to/v1/products', [
            'is_archived' => false,
        ]);
...
This endpoint will be responsible for creating a new checkout session, redirecting the user to the Solei Checkout page & redirect back to a configured confirmation page.
...
class CheckoutController extends Controller
{
    public function handle(Request $request)
    {
        ...
        // Solei will replace {CHECKOUT_ID} with the actual checkout ID upon a confirmed checkout
        ...
        ])->post('https://sandbox-api.solei.to/v1/checkouts/custom/', [
...
class ConfirmationController extends Controller
{
    public function handle(Request $request)
    {
        ...
        $data = Http::withHeaders([
            'Authorization' => 'Bearer ' . env('POLAR_API_KEY'),
            'Content-Type' => 'application/json',
        ])->get('https://sandbox-api.solei.to/v1/checkouts/custom/' . $request->query('checkout_id'));
...
Solei can send you events about various things happening in your organization. This is very useful for keeping your database in sync with Solei checkouts, orders, subscriptions, etc.
...
1. Point the Webhook to `your-app.com/api/webhook/solei`. This must be an absolute URL which Solei can reach. If you use ngrok, the URL will look something like this: `https://<your-ngrok-id>.ngrok-free.app/api/webhook/solei`.
...
return [
    'configs' => [
        [
            ...
            'signature_validator' => App\Handler\SoleiSignature::class,
...
Inside app/Handler/SoleiSignature.php, what we want to do is to validate that the request came from Solei. Add the code to that file.

```php
// app/Handler/SoleiSignature.php
<?php
...
class SoleiSignature implements SignatureValidator
{
...
If you have issues or need support, visit our [GitHub Discussions](https://github.com/nerd-zero/solei/discussions) or email [support@solei.to](mailto:support@solei.to).
