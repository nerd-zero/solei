...
description: "Learn how proration works in Solei, including available proration types, billing behavior during upgrades and downgrades, and how to configure proration using APIs and dashboard settings."
---
...
For both `prorate` and `invoice` proration types, the **subscription plan changes immediately upon successful payment**, whether the change is an upgrade or a downgrade. The only difference between the two proration types is **when the price difference is charged or credited**.

<Warning>
The subscription update is only applied if the payment succeeds. If the payment fails, an error is returned and the subscription remains unchanged.
</Warning>
...
<Note>
A new update always overrides any previous pending update that has not yet been applied. For example, if a `next_period` update was previously scheduled and a new update is then made with `invoice` or `prorate` proration, the pending update is discarded and the new update is applied immediately instead.
</Note>
...
<Steps>
    <Step title = "Go to Organization Settings">
        In the Solei dashboard sidebar, click on **Settings**.
            You can also go directly to:  
            `https://solei.to/dashboard/${org_slug}/settings`  
    ...
    <Step title = "Choose the Proration type">
        Choose the default proration behavior: **Invoice Immediately**, **Next invoice**, or **Apply on Next Period**.
...
You can change the default proration behavior using the [Update Organization API](https://solei.to/docs/api-reference/organizations/update).
    
## How to prorate when updating subscription via API

<Steps>
    <Step title = "Create an organization Token">
        Create a new organization token by following our [Organization Access Tokens](https://solei.to/docs/integrate/oat) guide.
    </Step>
    ...
    <Step title = "Copy the Subscription ID">
        - In the Solei dashboard sidebar, navigate to **Sales** > **Subscriptions** for your organization.
        You can also go directly to:  
        `https://solei.to/dashboard/${org_slug}/sales/subscriptions`
    ...
    <Step title = "Copy the Product ID">
        - In the Solei dashboard sidebar, navigate to **Products** > **Catalogue** for your organization.
        You can also go directly to:  
        `https://solei.to/dashboard/${org_slug}/products`  
    ...
    <Step title = "Call the Update Subscription API">
        ...
        Make sure to replace:
            ...
            - **type_of_proration** with the desired proration type:  
                        ...
                        - `next_period`: to schedule the change for the beginning of the next billing period (no proration)
<CodeGroup>

```bash cURL
curl --request PATCH \
  --url https://api.solei.to/v1/subscriptions/{<subscription_id>} \
...
```py Python
from solei_sdk import Solei

with Solei(
    access_token="<YOUR_BEARER_TOKEN_HERE>",
) as solei:

    res = solei.subscriptions.update(id="<subscription_id>", subscription_update={
        "product_id": "<product_id>",
        "proration_behavior": "<type_of_proration>"
    })
...
You can create a subscription with two plan options, one for \$5 and other for \$20 by following [Create Product Variants](https://solei.to/docs/guides/create-variants).  

### Case 1: Upgrading the subscription
You can upgrade a subscription by following the steps outlined in [Subscription Upgrades](https://solei.to/docs/guides/subscription-upgrades), which explains the process from both the merchant’s and the customer’s perspectives.
...
You can downgrade a subscription by following the steps outlined in [Subscription Downgrades](https://solei.to/docs/guides/subscription-downgrades), which explains the process from both the merchant’s and the customer’s perspectives.
