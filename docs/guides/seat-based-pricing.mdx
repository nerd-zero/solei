...
With standard Solei products, one person buys and one person uses — they're the same person. With seat-based products, buying and using are separate concerns, modeled through three distinct entities:
...
- Solei organization with seat-based pricing enabled
- Solei SDK installed (`npm install @solei/sdk` or `pip install solei-sdk`) — make sure it's the latest version
- Basic understanding of Solei products and subscriptions
...
<Steps>
  <Step title="Navigate to Products">
    In the Solei dashboard, go to **Products** and click **Create Product**.
  </Step>
  ...
  <Step title="Configure pricing">
    ...
    **Volume discounts** — the per-seat price is determined by the total seats purchased, and that rate applies to all seats. Example with the same tiers: buying 14 seats = 14 × $8 = $112 (all seats at the 11+ rate).
...
```typescript
const checkout = await solei.checkouts.create({
  products: ["prod_123"],
  seats: 5,
...
Solei automatically sends invitation emails to assigned team members with a secure claim link. Once a member claims their seat, benefits are granted immediately.

<Info>
Invitation emails are sent automatically when seats are assigned. The claim flow is fully managed by Solei — members click the link, claim their seat, and get immediate access to benefits.
</Info>
...
```typescript
const seat = await solei.customerSeats.assign({
...
```typescript
app.post('/webhooks/solei', async (req, res) => {
...
```typescript
await solei.subscriptions.update({
  id: subscriptionId,
...
| Claim link expired | Tokens expire after 24 hours. Resend via the portal or API: `solei.customerSeats.resend({ seat_id })` |
