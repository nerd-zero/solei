...
By passing either `customer_id` or `external_customer_id` when creating a checkout session, Solei will:
...
If you've already created a customer in Solei and have their Solei customer ID, you can use it directly.

<Steps>
    <Step title = "Get the Customer ID">
        Retrieve the customer ID from your Solei dashboard or through the [Customers API](/api-reference/customers/list).
...
const solei = new Solei({ accessToken: process.env["SOLEI_ACCESS_TOKEN"] });
...
with Solei(
    ...
    checkout = solei.checkouts.create(request={
        "products": ["<product_id>"],
...
```bash cURL
curl --request POST \
  --url https://api.solei.to/v1/checkouts/ \
...
If you have your own user management system, you can use your internal customer ID. This is the recommended approach as it makes reconciliation between your system and Solei easier.
