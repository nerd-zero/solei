...
description: "Learn how to grant meter credits to customers before they make any purchase using the Solei API."
---
...
1. Create a customer in Solei (if they don't exist)
...
<Steps>
  <Step title="Navigate to Meters">
    In the Solei dashboard sidebar, click on **Products** > **Meters**.
  </Step>
...
If your customer doesn't already exist in Solei, you need to create them first.

### Option A: Create via Dashboard

<Steps>
  <Step title="Navigate to Customers">
    In the Solei dashboard sidebar, click on **Customers**.
  </Step>
...
Use the Solei SDK or API to create a customer programmatically:
...
<Tip>
  Use the `externalId` field to link Solei customers with your internal user system. This allows you to reference customers without storing Solei's internal ID.
</Tip>

### Using External ID

If you set an `externalId` when creating the customer, you can use it in event ingestion instead of the Solei customer ID:
...
const meters = await solei.customerMeters.list({
