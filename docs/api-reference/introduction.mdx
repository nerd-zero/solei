...
<CardGroup cols={2}>
  <Card title="Production Base URL" icon="globe">
    `https://api.solei.to/v1`
  </Card>
  <Card title="Sandbox Base URL" icon="flask">
    `https://sandbox-api.solei.to/v1`
  </Card>
...
| ----------- | --------------------------------- | ------------------------------- |
| Production  | `https://api.solei.to/v1`         | Real customers & live payments  |
| Sandbox     | `https://sandbox-api.solei.to/v1` | Safe testing & integration work |
...
```http
Authorization: Bearer solei_oat_xxxxxxxxxxxxxxxxx
```
...
client = Solei(
    access_token=os.environ["SOLEI_ACCESS_TOKEN"],
    server="sandbox",
)
````
...
$sdk = Solei\Solei::builder()
    ->setServer('sandbox')
    ->setSecurity(getenv('SOLEI_ACCESS_TOKEN'))
    ->build();
```
...
List endpoints in the Solei API support pagination to help you efficiently retrieve large datasets. Use the `page` and `limit` query parameters to control pagination.
...
<CodeGroup>
```bash Request
curl https://api.solei.to/v1/products/?page=1&limit=100 \
  -H "Authorization: Bearer $POLAR_OAT" \
  -H "Accept: application/json"
```
...
Solei API has rate limits to ensure fair usage and maintain performance. Limits differ between the **Sandbox** and **Production** environments.
