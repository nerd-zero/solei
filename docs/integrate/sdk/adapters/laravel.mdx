...
Seamlessly integrate Solei.sh subscriptions and payments into your Laravel application. This package provides an elegant way to handle subscriptions, manage recurring payments, and interact with Solei's API. With built-in support for webhooks, subscription management, and a fluent API, you can focus on building your application while we handle the complexities of subscription billing.

<Note>
This provider is not maintained or officially supported by Solei. Use at your own discretion. If you have questions about the provider, please contact the project maintainer.
</Note>
...
```bash Terminal
composer require danestves/laravel-solei
```

**Step 2:** Run `:install`:

```bash Terminal
php artisan solei:install
```
...
```bash Terminal
php artisan vendor:publish --tag="solei-migrations"
```

```bash Terminal
php artisan vendor:publish --tag="solei-config"
```

```bash Terminal
php artisan vendor:publish --tag="solei-views"
```
...
return [
    ...
    |--------------------------------------------------------------------------
    |
    | The Solei access token is used to authenticate with the Solei API.
    | You can find your access token in the Solei dashboard > Settings
    | under the "Developers" section.
    |
    */
    'access_token' => env('SOLEI_ACCESS_TOKEN'),

    /*
    |--------------------------------------------------------------------------
    | Solei Webhook Secret
    |--------------------------------------------------------------------------
    |
    | The Solei webhook secret is used to verify that the webhook requests
    | are coming from Solei. You can find your webhook secret in the Solei
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | This is the base URI where routes from Solei will be served
    | from. The URL built into Solei is used by default; however,
    | you can modify this path as you see fit for your application.
    |
    */
    'path' => env('POLAR_PATH', 'solei'),
    ...
    | from a checkout session in your Solei account.
    |
    */
    'redirect_url' => null,
...
Configure your access token. Create a new token in the Solei Dashboard > Settings > Developers and paste it in the `.env` file.

- https://sandbox.solei.to/dashboard/ORG_SLUG/settings (Sandbox)
- https://solei.to/dashboard/ORG_SLUG/settings (Production)

```bash Terminal
SOLEI_ACCESS_TOKEN="<your_access_token>"
```

### Webhook Secret

Configure your webhook secret. Create a new webhook in the Solei Dashboard > Settings > Webhooks.

- https://sandbox.solei.to/dashboard/ORG_SLUG/settings/webhooks (Sandbox)
- https://solei.to/dashboard/ORG_SLUG/settings/webhooks (Production)
...
```php
use Danestves\LaravelSolei\Billable;
...
Solei includes a JavaScript script that you can use to initialize the [Solei Embedded Checkout](https://solei.to/docs/features/checkout/embed). If you going to use this functionality, you can use the `@soleiEmbedScript` directive to include the script in your views inside the `<head>` tag.
...
This package includes a webhook handler that will handle the webhooks from Solei.
...
Or if you're using Laravel v11 and up, you should exclude `solei/*` in your application's `bootstrap/app.php` file:
...
This package includes a list of commands that you can use to retrieve information about your Solei account.

| Command                      | Description                                |
| ---------------------------- | ------------------------------------------ |
| `php artisan solei:products` | List all available products with their ids |
...
```blade
<x-solei-button :checkout="$checkout" />
```
...
```blade
<x-solei-button :checkout="$checkout" data-solei-checkout-theme="dark" />
```
...
```php
public function soleiName(): ?string; // default: $model->name
public function soleiEmail(): ?string; // default: $model->email
```
...
Customers can update their personal information (e.g., name, email address) by accessing their [self-service customer portal](https://solei.to/docs/features/customer-portal). To redirect customers to this portal, call the `redirectToCustomerPortal()` method on your billable model (e.g., the User model).
...
```blade
@if ($order->refunded())
    Order {{ $order->solei_id }} was refunded on {{ $order->refunded_at->toFormattedDateString() }}
@endif
```
...
Solei does not offer immediate cancellation. To resume a subscription while it is still in its grace period, use the resume method.
...
Solei can send webhooks to your app, allowing you to react. By default, this package handles the majority of the work for you. If you have properly configured webhooks, it will listen for incoming events and update your database accordingly. We recommend activating all event kinds so you may easily upgrade in the future.

#### Webhook Events

- `Danestves\LaravelSolei\Events\BenefitGrantCreated`
- `Danestves\LaravelSolei\Events\BenefitGrantUpdated`
- `Danestves\LaravelSolei\Events\BenefitGrantRevoked`
- `Danestves\LaravelSolei\Events\OrderCreated`
- `Danestves\LaravelSolei\Events\OrderRefunded`
- `Danestves\LaravelSolei\Events\SubscriptionActive`
- `Danestves\LaravelSolei\Events\SubscriptionCanceled`
- `Danestves\LaravelSolei\Events\SubscriptionCreated`
- `Danestves\LaravelSolei\Events\SubscriptionRevoked`
- `Danestves\LaravelSolei\Events\SubscriptionUpdated`
...
use Danestves\LaravelSolei\Events\WebhookHandled;

class SoleiEventListener
{
    /**
     * Handle received Solei webhooks.
     */
...
The [Solei documentation](https://solei.to/docs/integrate/webhooks/events) includes an example payload.
...
use App\Listeners\SoleiEventListener;
...
Please see [CHANGELOG](https://github.com/danestves/laravel-solei/blob/main/CHANGELOG.md) for more information on what has changed recently.

## License

The MIT License (MIT). Please see [License File](https://github.com/danestves/laravel-solei/blob/main/LICENSE.md) for more information.
