...
A [Better Auth](https://github.com/better-auth/better-auth) plugin for integrating [Solei](https://solei.to) payments and subscriptions into your authentication flow.
...
- [Handle Solei Webhooks securely with signature verification](#webhooks-plugin)
- [Customer Portal](#portal-plugin)

## Examples

- [With Next.js, Better Auth and Cloudflare Workers](https://github.com/nerd-zero/solei-examples/tree/main/with-nextjs-better-auth-cloudflare-workers)

## Installation

Install the required Better Auth and Solei packages using the following command:

<Tabs>
  <Tab title="npm">
    ```bash Terminal
    npm install better-auth @solei/better-auth @solei/sdk
    ```
  </Tab>
  <Tab title="yarn">
    ```bash Terminal
    yarn add better-auth @solei/better-auth @solei/sdk
    ```
  </Tab>
  <Tab title="pnpm">
    ```bash Terminal
    pnpm add better-auth @solei/better-auth @solei/sdk
    ```
  </Tab>
  <Tab title="bun">
    ```bash Terminal
    bun add better-auth @solei/better-auth @solei/sdk
    ```
  </Tab>
</Tabs>

## Integrate Solei with BetterAuth

<Steps>
  <Step title="Configure Solei Access Token">
    Go to your Solei Organization Settings, create an Organization Access Token, and add it to the environment variables of your application.
  ...
  <Step title="Configure BetterAuth Server">
    The Solei plugin comes with it's own set of plugins to add functionality to your stack:
    ...
    - **webhooks** - Listen for relevant Solei webhooks

    ```typescript icon="square-js" BetterAuth Server with Solei Example
    import { betterAuth } from "better-auth";
    import { solei, checkout, portal, usage, webhooks } from "@solei/better-auth"; // [!code ++]
    import { Solei } from "@solei/sdk"; // [!code ++]

    const soleiClient = new Solei({ // [!code ++]
        accessToken: process.env.SOLEI_ACCESS_TOKEN, // [!code ++]
        // Use 'sandbox' if you're using the Solei Sandbox environment
    ...
    const auth = betterAuth({
        ...
        plugins: [
            solei({ // [!code ++]
                client: soleiClient, // [!code ++]
                createCustomerOnSignUp: true, // [!code ++]
                use: [ // [!code ++]
                    checkout({ // [!code ++]
                        products: [ // [!code ++]
                            { // [!code ++]
                                productId: "123-456-789", // ID of Product from Solei Dashboard // [!code ++]
    ...
    #### Solei Plugin Configuration Options
    ...
    const auth = betterAuth({
      ...
      plugins: [
        solei({
          ...
          use: [ // [!code ++]
            // This is where you add Solei plugins // [!code ++]
          ], // [!code ++]
        }),
      ],
    });
    ...
    - `client` (required): Solei SDK client instance
    - `createCustomerOnSignUp` (optional): Automatically create a Solei customer when a user signs up
    - `getCustomerCreateParams` (optional): Custom function to provide additional customer creation metadata
    - `use` (optional): Array of Solei plugins to enable specific functionality (checkout, portal, usage, and webhooks)

  </Step>
  <Step title="Configure BetterAuth Client">
    You will be using the BetterAuth Client to interact with the Solei functionalities.

    ```typescript icon="square-js" BetterAuth Client with Solei Example
    import { createAuthClient } from "better-auth/react";
    import { soleiClient } from "@solei/better-auth/client"; // [!code ++]
    import { organizationClient } from "better-auth/client/plugins"; // [!code ++]

    // All Solei plugins, etc. should be attached to BetterAuth server
    export const authClient = createAuthClient({ // [!code ++]
      plugins: [soleiClient()], // [!code ++]
    }); // [!code ++]
    ```
...
Enable the `createCustomerOnSignUp` [Solei plugin configuration option](#solei-plugin-configuration-options) to automatically create a new Solei Customer when a new User is added in the BetterAuth database.

All new customers are created with an associated `externalId`, i.e. the ID of your User in the Database. This skips any Solei to User mapping in your database.

## Sync Customer deletion

To add user deletion logic with external Solei customer deletion in BetterAuth, extend the user config of your betterAuth setup with the deleteUser option and an afterDelete hook. Here’s how to integrate this alongside your Solei plugin and automatic customer creation:
  ...
  user: {
    ...
    deleteUser: {
      ...
      afterDelete: async (user, request) => {
        // [!code ++]
        await solei.customers.deleteExternal({
          // [!code ++]
...
[Source code](https://github.com/nerd-zero/solei-adapters/blob/main/packages/solei-betterauth/src/plugins/checkout.ts)
...
<Steps>
  <Step title="Use Checkout Plugin">
    Update the `use` property of the Solei plugin for BetterAuth client to have the `checkout` plugin.

    ```typescript icon="square-js" Checkout Plugin Example
    import {
      solei,
      checkout // [!code ++]
    } from "@solei/better-auth";
  ...
  <Step title="Create checkouts using BetterAuth client">
    ...
    - **`products`** (optional): An array of Solei Product IDs.
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Solei Example
    ...
    ```typescript icon="square-js" BetterAuth Checkout with Solei Organization Example
    const organizationId = (await authClient.organization.list())?.data?.[0]?.id,

    await authClient.checkout({
        // Any Solei Product ID can be passed here
...
[Source code](https://github.com/nerd-zero/solei-adapters/blob/main/packages/solei-betterauth/src/plugins/usage.ts)
...
Solei's Usage Based Billing builds entirely on event ingestion. Ingest events from your application, create Meters to represent that usage, and add metered prices to Products to charge for it.
...
[Source code](https://github.com/nerd-zero/solei-adapters/blob/main/packages/solei-betterauth/src/plugins/webhooks.ts)

The `webhooks` plugin can be used to capture incoming events from your Solei organization.

To set up the Solei `webhooks` plugin with the BetterAuth client, follow the steps below:

<Steps>
  <Step title="Configure Webhook Endpoints in Solei">
    Configure a Webhook endpoint in your Solei Organization Settings page by following [this guide](/integrate/webhooks/endpoints). Webhook endpoint is configured at `/api/auth/solei/webhooks`.
  </Step>
...
The `webhooks` plugin allows you to invoke handlers for all Solei webhook events:
...
[Source code](https://github.com/nerd-zero/solei-adapters/blob/main/packages/solei-betterauth/src/plugins/portal.ts)
...
The following method will redirect the user to the Solei Customer Portal, where they can see their orders, purchases, subscriptions, benefits, etc.
...
Learn more about the Solei Customer State [in the Solei Docs](https://solei.to/docs/integrate/customer-state).
