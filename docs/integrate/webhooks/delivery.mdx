...
You now need to setup a route handler for the endpoint registered on Solei to
...
import { validateEvent, WebhookVerificationError } from '@solei/sdk/webhooks'
...
from solei_sdk.webhooks import validate_event, WebhookVerificationError
...
Seeing deliveries on Solei, but not receiving them on your end? Below are some
...
- Try adding trailing `/` to the URL on Solei. Often `/foo` is resolved to
  `/foo/` by frameworks.

`HTTP 3xx`

Redirect responses (301, 302, 307, etc.) are treated as failures. Solei does not follow redirects for webhook deliveries. Update your webhook URL to the final destination URL to avoid redirects.
