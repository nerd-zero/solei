...
2. Solei Checkout: Buyer enters email + card
3. Solei auto-creates individual customer
4. Post-purchase: Customer manages subscription via Customer Portal

#### Workflow B: Public Checkout (B2B - Current seats flow)
1. Customer visits merchant site, clicks checkout link
2. Solei Checkout: Buyer enters email + card
3. Solei auto-creates billing manager
...
2. Merchant creates Solei customer + business inside Solei via API
3. Billing manager clicks checkout in merchant app
4. Solei Checkout: Pre-filled information, enters card
...
5. **Solei Developer Experience** (weight: 3) - Avoid excessive filtering or complex queries
...
| Solei Dev Experience    | 🟢 3  | Clean queries, no type filtering, clear separation |
...
| Solei Dev Experience    | 🟡 2  | Requires type filtering, mitigated by SQLAlchemy polymorphism |
...
| Solei dev experience    | 3      | 🟢 3 (9)               | 🟡 2 (6)               |
...
2. **Auto-create default members**: Migration script creates one member per customer
   - `member.customer_id` → existing customer ID
   - `member.email` → customer email
   - `member.name` → customer name
   - `member.external_id` → customer external_id (if exists)
   - `member.role` → `"owner"` (default)
3. **Service layer dual support**: Endpoints accept both `customer_id` (legacy) and `member_id` (new)
   - When `customer_id` provided → resolve to default member automatically
   - When `member_id` provided → use directly
4. **Migrate usage data**:
   - `benefit_grants.member_id` → default member ID (keep `customer_id` for backward compat queries)
   - `customer_sessions.member_id` → default member ID
   - `events.member_id` → default member ID (keep `customer_id` denormalized)
   - `customer_seats.member_id` → NULL initially (migrated in Phase 2)
...
4. **Migrate usage data**:
   ...
   - **Events**: Reassign from seat holder customer → member (update both `member_id` and keep `customer_id` pointing to billing customer)
...
5. **Update merchant integrations** (⚠️ **Breaking changes**):
   ...
   **📖 See [Appendix A: Merchant Migration Guide](#appendix-a-merchant-migration-guide-b2bseat-based-products) for detailed instructions, code examples, and FAQ.**
...
We're introducing a new **Member** entity to improve how Solei handles team subscriptions and usage-based billing. This change provides:
...
**Problem we're solving**: When a user (e.g., Alice) works for multiple companies (Acme Corp and Slack Inc), and both companies purchase your product, Solei doesn't know which company to bill when Alice generates usage events. This causes billing ambiguity. This will allow other futures in the comming weeks, like better analytics and insights.
...
const session = await solei.customerSessions.create({
...
**Before** (current):
```typescript
const session = await solei.benefits.grants({
  externalCustomerId: "cust_ben_123"
});
```
...
- [ ] **Update SDK to latest version**
  - TypeScript: `npm install @solei/sdk@latest`
  - Python: `pip install --upgrade solei-sdk`
...
**You ARE affected if**:
- ✅ You sell products with seat-based pricing
- ✅ You use the Customer Seats API (`/v1/customer-seats`)
- ✅ You have subscriptions or orders with `seats > 0`
- ✅ You track usage events for team members
...
| **Week 1-2** | One-on-one migration support | Work with Solei team if issues arise |
...
- Email support@solei.sh with questions

**During migration**:
- Schedule 1-on-1 call if needed.

**After migration**:
- Report issues immediately: support@solei.sh
- Check status page: status.solei.sh
...
This appendix shows how merchants integrate with Solei for common workflows, comparing **Option 1 (Member Model)** vs **Option 2 (Customer Type + Member)** side-by-side.
...
```http
POST https://api.solei.sh/v1/customers
Authorization: Bearer solei_secret_...
...
POST https://api.solei.sh/v1/customers/cust_acme_123/members
Authorization: Bearer solei_secret_...
...
POST https://api.solei.sh/v1/checkouts
Authorization: Bearer solei_secret_...
...
{
  "id": "checkout_abc",
  "url": "https://solei.sh/checkout/abc",
  "customer_id": null  // Created after payment
}
```

```http
# Step 2: Customer completes checkout (Solei hosted)
# → Solei auto-creates Customer + Member
...
# Solei automatically creates BenefitGrant linked to ben_alice_789
```
...
```http
# Step 2: Customer completes checkout (Solei hosted)
# → Solei auto-creates Customer with type="individual"
...
# Solei automatically creates BenefitGrant linked to cust_alice_456
```
...
{
  "id": "checkout_def",
  "url": "https://solei.sh/checkout/def",
  "customer_id": "cust_acme_123"
}
```
...
# → Solei auto-creates member for billing@acme.com
```
...
POST https://api.solei.sh/v1/subscriptions/sub_acme_456/seats
Authorization: Bearer solei_customer_session_... // ben_billing_789 session
...
```http
# Step 5: Beneficiaries claim seats
POST https://api.solei.sh/v1/subscriptions/sub_acme_789/seats
Authorization: Bearer solei_customer_session_... // cust_acme_456 session
...
https://buy.solei.sh/solei_cl_gon2ISU4r7oYyjVaxXlRVUv5MKh1H3c1gCxKn1zZGHA
...
```http
# Merchant shares public checkout link
https://solei.sh/org_abc/products/prod_team_plan

# Customer clicks, enters email, completes purchase
# → Solei detects seat-based product, auto-creates:
...
GET https://api.solei.sh/v1/customer-portal/downloadables
Authorization: Bearer solei_customer_session_...
...
GET https://api.solei.sh/v1/benefit-grants?member_id=ben_alice_acme_123
Authorization: Bearer solei_member_session_...
```
...
```http
# Download benefit
GET https://api.solei.sh/v1/benefits/benefit_theme_download/download
Authorization: Bearer solei_customer_session_...
```
...
POST https://api.solei.sh/v1/customer-sessions
Authorization: Bearer solei_secret_...
...
```json
{
  "token": "solei_cst_abc123xyz",
  "customer_id": "cust_bob_123",
  "member_id": "ben_bob_default_456",  // Auto-resolved from customer
  "return_url": "https://solei.sh/org_merchant/portal?token=solei_cst_abc123xyz",
...
```json
{
  "token": "solei_cst_def456",
  "member_id": "ben_jane_789",
  "customer_id": "cust_acme_123",
  "url": "https://solei.sh/portal?token=solei_cst_def456",
  "expires_at": "2025-12-01T10:00:00Z"
}
```
...
```http
# Option A: Using customer_id (B2C backward compatible)
POST https://api.solei.sh/v1/events
Authorization: Bearer solei_secret_...
...
**What happens:**
- Solei checks if cust_alice_123 is a member of any businesses
...
The user access: https://solei.sh/petru-test/portal/request
