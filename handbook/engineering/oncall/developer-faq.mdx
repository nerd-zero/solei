...
1. Clone the `solei-js` repository
2. In `solei-js/.speakeasy/workflow.yaml` change `https://api.solei.sh/openapi.json` to `http://127.0.1:8000/openapi.json`
...
5. In the main solei repo, go to `clients/apps/web` and install the local dependency with `pnpm` e.g. `pnpm add "/Users/<user>/Development/solei-js"` (or whatever your local path to the `solei-js` repository is).
