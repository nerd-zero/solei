...
description: "Best practices and conventions for designing and implementing RESTful APIs at Solei"
---

This document outlines the guidelines and principles we follow when designing and implementing RESTful APIs at Solei. Adhering to these standards ensures consistency, maintainability, and a better developer experience for both internal and external users of our APIs.
...
- Always use `solei.kit.Schema` (or `TimestampedSchema` when timestamps exist)
...
- Validation errors should return `422 Unprocessable Entity` in FastAPI format via `SoleiRequestValidationError`
...
**Inherit from `solei.kit.Schema`**

```py
# ✅ DO
from solei.kit import Schema
...
**If the underlying data model includes `created_at` or `updated_at` fields, inherit from `solei.kit.TimestampedSchema`**

```py
# ✅ DO
from solei.kit import TimestampedSchema
...
- On validation errors, return `422 Unprocessable Entity` with FastAPI-style details (`SoleiRequestValidationError`).
