...
description: "Understanding and handling operational errors in Solei backend development"
---
...
1. **Create a matcher function** in `server/solei/operational_errors.py`:
...
- **API requests**: `OperationalErrorMiddleware` in `server/solei/middlewares.py`
- **Background jobs**: `OperationalErrorMiddleware` in `server/solei/worker/_broker.py`
